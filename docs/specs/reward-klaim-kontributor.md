# Reward Klaim Kontributor (Khadam Syaikhuna)
...
Sistem Kontributor sudah berjalan: kontribusi yang disetujui admin menambah `users.total_khidmah_points` (XP) dan menaikkan `users.badge_title` secara otomatis melalui `User::updateBadge()` (`Jamaah Aktif` < 101 ≤ `Penuntut Ilmu` < 501 ≤ `Khadam Syaikhuna`). Saat ini badge tertinggi **`Khadam Syaikhuna` (≥ 501 XP)** hanya berupa gelar kehormatan tanpa insentif nyata.

Fitur ini menambahkan **reward uang tunai** (default **Rp 50.000**) yang dapat **diklaim** oleh kontributor saat mencapai threshold XP `Khadam Syaikhuna`. Reward **ditransfer manual oleh admin** melalui e-wallet; aplikasi hanya mengelola pengajuan, status, dan audit trail — **tidak** ada integrasi payment gateway.

---

## Tujuan

1. Memberi insentif konkret bagi kontributor yang mencapai gelar `Khadam Syaikhuna` untuk mendorong kontribusi berkualitas.
...
- Tidak menambah reward untuk badge selain `Khadam Syaikhuna` (tidak ada tier reward bertingkat).
...
- `User::updateBadge()` (`app/Models/User.php:152`) menetapkan `badge_title = 'Khadam Syaikhuna'` saat `total_khidmah_points >= 501`. Tidak ada efek samping lain.
...
min_xp      integer        default 501     -- threshold XP minimal (selaras Khadam Syaikhuna)
...
- **CC-01:** Threshold reward memakai kolom numerik `min_xp`, **independen** dari logika `badge_title` di `User::updateBadge()`. Mengubah `min_xp` tidak memengaruhi badge, dan sebaliknya. Default `501` menyelaraskan reward dengan badge `Khadam Syaikhuna`.
...
5. Coba akses URL bukti transfer sebagai user lain (bukan pemilik/admin) → `403`/`404`; sebagai pemilik klaim atau admin → gambar tampil.
6. Buka `/kontributor/saya` lagi → "Reward sudah diterima"; coba POST store langsung → ditolak (sekali seumur hidup).
7. Pada kontributor lain: ajukan klaim → admin tolak dengan alasan → kontributor melihat alasan & kartu klaim ulang → ajukan ulang berhasil.
8. Ubah pengaturan: `amount` & `min_xp` & matikan `is_active` → verifikasi kartu klaim hilang dan klaim baru memakai nilai baru saat diaktifkan kembali.
9. Revoke kontribusi kontributor yang punya klaim `pending` → verifikasi klaim tetap dapat diproses (snapshot).
...
**Definition of Done:** seluruh AC-01..AC-11 terpenuhi, test/lint hijau, diff bersih (lihat _Definition of Done_ CLAUDE.md).
