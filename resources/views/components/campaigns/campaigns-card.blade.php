...
<div class="col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-xs rounded-xl">
    <div class="flex flex-col h-full p-5">
                ...
                <div class="flex shrink-0 -space-x-3 -ml-px">
                    @foreach ($campaign->members as $member)                        
                        <a class="block" href="{{ $member->link }}">
                            <img class="rounded-full border-2 border-white dark:border-gray-800 box-content" src="{{ asset('images/' . $member->image) }}" width="28" height="28" alt="{{ $member->name }}" />
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <footer class="mt-5">
            <div class="text-sm font-medium text-gray-500 mb-2">{{ \Carbon\Carbon::parse($campaign->start_date)->format('M j') }} <span class="text-gray-400 dark:text-gray-600">-&gt;</span> {{ \Carbon\Carbon::parse($campaign->end_date)->format('M j') }}</div>
