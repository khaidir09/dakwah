    ...
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-[96rem] mx-auto">
        ...
        <div class="sm:flex sm:justify-between sm:items-center mb-8">
            <div class="mb-4 sm:mb-0">
                <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Tambah Jadwal Majelis</h1>
            </div>
        ...
        <div>
            <form action="{{ route('jadwal-majelis.store') }}" method="POST">
                @csrf
                <div class="bg-white dark:bg-gray-800 p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                        ...
                        <div>
                            ...
                            <select id="assembly_id" class="form-select w-full @error('assembly_id') is-invalid @enderror" name="assembly_id" required>
                                ...
                                @foreach($majelis as $item)
                                    <option value="{{ $item->id }}">{{ $item->nama_majelis }}</option>
                                @endforeach
                            </select>
                            @error('assembly_id')
                                <div class="text-xs mt-1 text-red-500">{{ $message }}</div>
                    ...
                    <div class="grid grid-cols-1 mt-6">
                        <div>
                            ...
                            <textarea class="form-textarea w-full @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" cols="30" rows="10">{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <div class="text-xs mt-1 text-red-500">{{ $message }}</div>
                            @enderror
                        </div>
