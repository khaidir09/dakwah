            ...
            <form action="{{ route('majelis.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="bg-white dark:bg-gray-800 p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div x-data="{ manual: {{ old('custom_leader_name') ? 'true' : 'false' }} }">
                            ...
                            <div x-show="!manual">
                                <select id="teacher_id" class="form-select w-full @error('teacher_id') is-invalid @enderror" name="teacher_id" :disabled="manual">
                                    ...
                                    @foreach($teachers as $item)
                                        <option value="{{ $item->id }}" {{ old('teacher_id') == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                    @endforeach
                                </select>
                                @error('teacher_id')
                                    <div class="text-xs mt-1 text-red-500">{{ $message }}</div>
                            ...
                            <div x-show="manual">
                                <input id="custom_leader_name" class="form-input w-full @error('custom_leader_name') is-invalid @enderror" type="text" name="custom_leader_name" value="{{ old('custom_leader_name') }}" placeholder="Masukkan nama pimpinan..." :disabled="!manual" />
                    ...
                    <div class="grid grid-cols-1 mt-6">
                        <div>
                            ...
                            <textarea class="form-textarea w-full @error('deskripsi') is-invalid @enderror" name="deskripsi" id="deskripsi" cols="30" rows="10" required>{{ old('deskripsi') }}</textarea>
                            @error('deskripsi')
                                <div class="text-xs mt-1 text-red-500">{{ $message }}</div>
                            @enderror
                        </div>
