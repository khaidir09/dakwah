<x-dashboard-layout>
    <div class="grow" x-data="{
        sections: {{ Js::from(old('sections', $article->sections)) }},
        citations: {{ Js::from(old('citations', $article->citations)) }},
        bibliography: {{ Js::from(old('bibliography', $article->bibliography)) }},
        addSection() {
        ...
        <div class="p-6 space-y-6">
            <div class="flex justify-between items-center mb-5">
                <h2 class="text-2xl text-gray-800 dark:text-gray-100 font-bold">Edit Artikel Ilmiah</h2>
            ...
            <form action="{{ route('kelola-artikel.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid md:grid-cols-2 gap-6 mb-6">
                        ...
                        @if($foundations->count() > 1)
                            <select id="foundation_id" class="form-select w-full @error('foundation_id') is-invalid @enderror" name="foundation_id" required>
                                ...
                                @foreach($foundations as $foundation)
                                    <option value="{{ $foundation->id }}" {{ old('foundation_id', $article->foundation_id) == $foundation->id ? 'selected' : '' }}>{{ $foundation->name }}</option>
                                @endforeach
                            </select>
                        @else
                    ...
                    <div>
                        ...
                        <input id="published_at" class="form-input w-full @error('published_at') is-invalid @enderror" type="datetime-local" name="published_at" value="{{ old('published_at', $article->published_at ? \Carbon\Carbon::parse($article->published_at)->format('Y-m-d\TH:i') : '') }}"/>
                        @error('published_at')
                            <div class="text-xs mt-1 text-red-500">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-medium mb-2" for="cover_image">Gambar Sampul</label>
                         @if($article->cover_image)
                            <div class="mb-4">
                                <img src="{{ Storage::url($article->cover_image) }}" alt="Cover Artikel" class="w-32 h-auto rounded-lg shadow-sm border border-gray-200">
