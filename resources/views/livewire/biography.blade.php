<div>
    ...
    <div class="sm:flex sm:justify-between sm:items-center mb-8">
        ...
        <div class="mb-4 sm:mb-0">
            <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Manaqib</h1>
        </div>
        ...
        <div class="grid grid-flow-col sm:auto-cols-max justify-start sm:justify-end gap-2">
            ...
            <!-- Search form -->
            <x-search-form placeholder="Masukkan nama" />

            <!-- Add button -->
            <a href="{{ route('biographies.create') }}" class="btn bg-gray-900 text-gray-100 hover:bg-gray-800 dark:bg-gray-100 dark:text-gray-800 dark:hover:bg-white">
                <svg class="fill-current shrink-0 xs:hidden" width="16" height="16" viewBox="0 0 16 16">
                    <path d="M15 7H9V1c0-.6-.4-1-1-1S7 .4 7 1v6H1c-.6 0-1 .4-1 1s.4 1 1 1h6v6c0 .6.4 1 1 1s1-.4 1-1V9h6c.6 0 1-.4 1-1s-.4-1-1-1z" />
                </svg>
                <span class="max-xs:sr-only">Tambah Manaqib</span>
            </a>
    ...
    <div class="bg-white dark:bg-gray-800 shadow-xs rounded-xl">
        <header class="px-5 py-4">
            <h2 class="font-semibold text-gray-800 dark:text-gray-100">Semua Data <span class="text-gray-400 dark:text-gray-500 font-medium">{{ $biographies_count }}</span></h2>
        </header>
            ...
            <table class="table-auto w-full dark:text-gray-300">
                ...
                <thead class="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/20 border-t border-b border-gray-100 dark:border-gray-700/60">
                    <tr>
                        ...
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                            <div class="font-semibold text-left">Wafat</div>
                        </th>
                        <th class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                ...
                <tbody class="text-sm divide-y divide-gray-100 dark:divide-gray-700/60">
                    ...
                    @foreach($biographies as $item)
                        <tr>
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="text-left">
                                    @if ($item->tanggal_wafat_masehi)
                                        <div>Masehi: {{ \Carbon\Carbon::parse($item->tanggal_wafat_masehi)->translatedFormat('d F Y') }}</div>
                                    @endif
                                    @if ($item->tanggal_wafat_hijriah)
                                        <div class="text-xs text-gray-500">Hijriah: {{ $item->tanggal_wafat_hijriah }}</div>
                                    @endif
                                </div>
                            </td>
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
                                <div class="text-left">
                                    @if ($item->foto)
                                        <img class="w-16 h-16 object-cover rounded-md" src="{{ Storage::url($item->foto) }}" alt="{{ $item->nama }}">
                                    @else
                            ...
                            <td class="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
                                <div class="space-x-1 flex">
                                    <a href="{{ route('biographies.edit', $item->id) }}" class="text-gray-400 hover:text-gray-500 dark:text-gray-500 dark:hover:text-gray-400 rounded-full">
                                ...
                                <div class="p-5 flex space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <div class="flex flex-wrap justify-end space-x-2">
                                            ...
                                            <button
                                                wire:click="deleteBiography"
                                                wire:loading.attr="disabled"
                                                wire:target="deleteBiography"
                                                class="btn-sm bg-red-500 hover:bg-red-600 text-white"
                                            >
                                                <span wire:loading.remove wire:target="deleteBiography">Ya, Hapus</span>
                                                <span wire:loading wire:target="deleteBiography">Menghapus...</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
