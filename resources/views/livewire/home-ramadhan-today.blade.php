<div>
    @if($lectures->count() > 0)
        <!-- Title -->
        <header class="mb-4">
            <h1 class="text-2xl md:text-3xl text-gray-800 dark:text-gray-100 font-bold">Jadwal Ramadhan Hari Ini</h1>
        </header>

        <div class="grid grid-cols-12 gap-4 mb-8">
            @foreach ($lectures as $lecture)
                <div class="col-span-full xl:col-span-6 bg-white dark:bg-gray-800 shadow-xs rounded-xl overflow-hidden border border-emerald-100 dark:border-emerald-900/30">
                    <div class="flex flex-col h-full">
                        ...
                        <div class="grow p-5">
                            <div class="flex justify-between items-start">
                                ...
                                <header class="w-full">
                                    <div class="flex mb-2">
                                        @if($lecture->teacher)
                                            <a class="relative inline-flex items-start mr-5 shrink-0" href="{{ route('guru-detail', $lecture->teacher) }}">
                                                @if($lecture->teacher->foto != null)
                                                    <img class="rounded-full w-16 h-16 object-cover" src="{{ Storage::url($lecture->teacher->foto) }}" alt="{{ $lecture->teacher->name }}" />
                                                @else
                                            ...
                                            <div class="mt-1 pr-1 w-full">
                                                <a class="inline-flex text-gray-800 dark:text-gray-100 hover:text-gray-900 dark:hover:text-white" href="{{ route('guru-detail', $lecture->teacher) }}">
                                                    <h2 class="text-xl leading-snug justify-center font-semibold">{{ $lecture->teacher->name }}</h2>
                                                </a>
                                                <div class="text-sm text-gray-500 line-clamp-2">{{ $lecture->title }}</div>
                                            </div>
                                        @else
                                            <!-- Custom Speaker -->
                                            <div class="relative inline-flex items-start mr-5 shrink-0">
                                                 <div class="w-16 h-16 flex items-center justify-center bg-emerald-100 dark:bg-emerald-900/50 rounded-full text-emerald-500">
                                                    <svg class="w-8 h-8 fill-current" viewBox="0 0 24 24">
                                                        <path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" />
                                                    </svg>
                                                </div>
                                            ...
                                            <div class="mt-1 pr-1 w-full">
                                                <h2 class="text-xl leading-snug justify-center font-semibold text-gray-800 dark:text-gray-100">{{ $lecture->custom_speaker_name ?? 'Penceramah Tamu' }}</h2>
                                                <div class="text-sm text-gray-500 line-clamp-2">{{ $lecture->title }}</div>
                                            </div>
                                        @endif
                                    </div>
                        ...
                        <div class="border-t border-gray-100 dark:border-gray-700/60">
                            <div class="flex divide-x divide-gray-100 dark:divide-gray-700/60">
                                <a class="block flex-1 text-center text-sm text-emerald-500 hover:text-emerald-600 dark:hover:text-emerald-400 font-medium px-3 py-4" href="{{ route('majelis-detail', $lecture->schedule->assembly->id) }}">
                                    <div class="flex items-center justify-center">
                                        ...
                                        <span>{{ $lecture->schedule->assembly->nama_majelis }}</span>
                                    </div>
                                </a>
